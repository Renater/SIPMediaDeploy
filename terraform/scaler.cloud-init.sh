...
SCALER_REPLICAS=${scaler_replicas}
SCALER_IP=$(hostname -I | grep -o -E "([0-9]{1,3}\.){3}[0-9]{1,3}" 2>&1 | head -n 1)
...
sed -i -E "s/^AUTOSCALING_MIN_REPLICA=[0-9]*$/AUTOSCALING_MIN_REPLICA=$SCALER_REPLICAS/" .env
sed -i -E "s/^AUTOSCALING_HOST_IP_ADDRESS=.*$/AUTOSCALING_HOST_IP_ADDRESS=$SCALER_IP/" .env
